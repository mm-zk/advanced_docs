...
contract. These transactions are referred to as [`L1Tx`][l1_tx] transactions, and the process of sending transactions in
this manner is called the 'priority queue'.
...
This method takes the next transaction from the mempool (which could be either an L1Tx or L2Tx), executes it, and checks
if the L1 batch is ready to be sealed (for more details on when we finalize L1 batches, see the 'Blocks & Batches'
article).
